...
Raspberry Pi 5
   ↓
Audio interface / amplifier
   ↓
66FR speaker (8 Ω, 0.5 W)
   ↓
...
The Raspberry Pi 5 does not have a built-in speaker or 3.5 mm analog audio jack, so the kiosk uses an external audio output/interface. The current kiosk speaker is a **66FR, 8 Ω, 0.5 W speaker** and should be driven through a suitable audio amplifier/interface rated for an 8 Ω load. Do not connect the speaker directly to a GPIO pin.
...
- **66FR speaker (8 Ω, 0.5 W)** — kiosk TTS/navigation audio through a suitable audio amplifier/interface
...
Kiosk audio is generated by the browser's Web Speech API/TTS and sent to the external **66FR 8 Ω, 0.5 W speaker** through the Raspberry Pi's configured audio output and a suitable amplifier/interface.
...
- `0001.mp3` — general boarding announcement
- `0002.mp3` — ramp ready
- `0003.mp3` — general alighting announcement
...
WAIT 30 seconds
        ↓
TTS 0001.mp3 — boarding announcement
        ↓
Announcement finishes
        ↓
Ramp deploys
        ↓
TTS 0002.mp3 — ramp ready
        ↓
Passenger boards
```
...
WAIT 30 seconds
        ↓
TTS 0003.mp3 — alighting announcement
        ↓
Announcement finishes
        ↓
Ramp deploys
...
RETRACT signal received
        ↓
TTS 0004.mp3 — retracting announcement
        ↓
Announcement finishes
        ↓
Ramp retracts
...
The Raspberry Pi 5 uses the **66FR 8 Ω, 0.5 W speaker** for kiosk TTS. Use a supported Raspberry Pi audio output such as USB, HDMI, Bluetooth or another suitable audio interface, with an amplifier appropriate for an 8 Ω, 0.5 W speaker.
...
- correct amplifier/interface connection for the 8 Ω, 0.5 W kiosk speaker
